...
void executeCommand(uint8_t *tx_buffer) {
	...
	/* PMOD ACL variables */
	uint8_t pmod_acl_read_data;
	uint32_t pmod_acl_xydata;
	uint8_t pmod_acl_intr_status;
	...
	// --------------------------------------------------------------------------------- //
	// PMOD_ACL_WRITE_BYTE: Write to PmodACL Register
	// Field 1 = address ; Field 2 = data
	// --------------------------------------------------------------------------------- //
	case PMOD_ACL_WRITE_BYTE:
		/* Write the data and update the response buffer */
		pmodAcl_WriteByte((uint8_t) field1, (uint8_t) field2);
		setResponseBytes(tx_buffer, PMODACL_WRITE_BYTE_OKAY);
		break;


	// --------------------------------------------------------------------------------- //
	// PMOD_ACL_READ_BYTE: Read from PmodACL Register
	// Field 1 = PmodACL register address
	// --------------------------------------------------------------------------------- //
	case PMOD_ACL_READ_BYTE:
		/* Write the data and update the response buffer */
		pmod_acl_read_data = pmodAcl_ReadByte((uint8_t) field1);
		setResponseBytes(tx_buffer, (uint32_t) pmod_acl_read_data);
		break;


	// --------------------------------------------------------------------------------- //
	// PMOD_ACL_READ_XYDATA: Read back PmodACL XY-Data
	// Field 1 = n/a, Field 2 = n/a
	// --------------------------------------------------------------------------------- //
	case PMOD_ACL_READ_XYDATA:
		/* Write the data and update the response buffer */
		pmod_acl_xydata = pmodAcl_ReadXYData();
		setResponseBytes(tx_buffer, pmod_acl_xydata);
		break;


	// --------------------------------------------------------------------------------- //
	// PMOD_ACL_READ_INTR_STATUS: Read back PmodACL interrupt status register
	// Field 1 = n/a, Field 2 = n/a
	// --------------------------------------------------------------------------------- //
	case PMOD_ACL_READ_INTR_STATUS:
		/* Write the data and update the response buffer */
		pmod_acl_intr_status = pmodAcl_ReadIntrStatus();
		setResponseBytes(tx_buffer, pmod_acl_intr_status);
		break;
