...
int sys_init(void){
	...
	// For SCU timer, must get pointer to XScuTimer instance:
	p_InitStatus->xscu_timer = xScuTimerInit(&p_xscutimer_inst);	// SCU TIMER
	...
	p_addIntrStatus->xscu_timer = addScuTimerToInterruptSystem(p_xscutimer_inst);
	...
	printf("SCU Timer initialization: ");
	if (p_InitStatus->xscu_timer != XST_SUCCESS) 	{ printf("Error detected.\n\r\n\r"); }
	else											{ printf("Success.\n\r\n\r"); }

	printf("Adding SCU Timer to interrupt system: ");
	if (p_addIntrStatus->xscu_timer != XST_SUCCESS) { printf("Error detected.\n\r\n\r"); }
	else											{ printf("Success.\n\r\n\r"); }
	...
	if (	(p_InitStatus->xscu_gic == XST_SUCCESS)			// XSCUGIC
		...
		&& 	(p_InitStatus->xscu_timer == XST_SUCCESS) )		// SCU TIMER
    {
	...
	/* Interrupt System */
	int add_intr_result;
	if (p_addIntrStatus->xscu_timer == XST_SUCCESS)
    {
